...
console.log("Array length:", evenNumArr.length);
...
console.log("oddSumCheck:", oddSumCheck);
console.log("oddSumCheck length:", oddSumCheck.length);
...
function nap(schedule){
  for(var i = 0; i < schedule.length; i++){
    if(schedule[i] === true){
      schedule.splice(i, 1, 'ZzZzZzZz');
    }else{
      schedule.splice(i, 1, 'Gotta get coding!');
    }
  }
...
function copyArray(originArray, destinationArray){
  for(var i = 0; i < originArray.length; i++){
    destinationArray.splice(i, 0, originArray[i]);
  }
...
var topQuote = "I've spent the first half of my life acquiring all this stuff and now I'll spend the second half getting rid of it!";
var topQuoteArray = [];
var word = 0;
var arrIndex = 0;

function longestWord(str, arr, len, indx){
  var arr = str.split(" ");
...
longestWord(topQuote, topQuoteArray, word, arrIndex);
console.log(topQuote.length);
...
allGrownUp(myWay, myWayArray, myWayDone);
